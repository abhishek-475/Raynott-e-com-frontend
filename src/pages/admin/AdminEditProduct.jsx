...
export default function AdminEditProduct() {
  ...
  const [specsArray, setSpecsArray] = useState([{ key: '', value: '' }]);
  ...
  const fetchProduct = async () => {
    try {
      ...
      setFormData({
        ...
        originalPrice: productData.originalPrice || productData.comparePrice || '',
  ...
  const handleSpecChange = (index, field, value) => {
    ...
    setSpecsArray(filteredSpecs);
    
    // Convert specs array to object for form data
    const specsObject = filteredSpecs
  ...
  const removeSpec = (index) => {
    ...
    // Update form data
    const specsObject = updatedSpecs
      .filter(spec => spec.key.trim() !== '' && spec.value.trim() !== '')
      .reduce((obj, spec) => {
        obj[spec.key.trim()] = spec.value.trim();
        return obj;
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      // Implement your image upload endpoint
            ...
            >
              {saving ? (
                ...
                <>
                  ...
                  {formData.status === 'draft' ? 'Save as Draft' : 'Update Product'}
                </>
              )}
            </button>
          </div>
